...
隐私上全部本地运行：没有后端、遥测、analytics、API key、云同步和远程代码；捕获永远由你手动触发。扩展只申请四项权限——`activeTab`、`scripting`、`sidePanel`、`storage`，没有 host permissions，没有 `<all_urls>`，没有 tabs、cookies、history。会话状态只写在 `chrome.storage.session`，关闭浏览器即清空，抓取的页面内容不写入 `chrome.storage.local`。项目配有完整的单元/集成测试套件与 13 个 Playwright 浏览器 E2E；v1.1.0 发布门禁均已通过。
